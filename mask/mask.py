...
def create_padding_mask(seq, pad_token=0):
    ...
    :param seq: 输入序列，形状：(batch_size, seq_len_enc)
    :param pad_token: 填充的TokenId，默认为0
    :return: PaddingMask (batch_size, 1, 1, seq_len_enc)
    """
    mask = (seq.eq(pad_token)).unsqueeze(1).unsqueeze(2)
    mask.to(choose_device())
    return mask
...
def create_causal_mask(seq_len):
    ...
    :param seq_len: 序列长度
    :return: (1, seq_len_dec, seq_len_enc)
    """
...
def create_combine_mask(padding_mask, causal_mask):
    ...
    """
    padding_mask = padding_mask.to(choose_device())
    causal_mask = causal_mask.to(choose_device())
